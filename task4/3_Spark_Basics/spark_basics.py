...
df_people = spark.read.option('header', 'true').schema(people_schema).csv('/home/jovyan/work/3_Spark_Basics/data/top_100_richest.csv')
df_country = spark.read.option('header', 'true').schema(country_schema).csv('/home/jovyan/work/3_Spark_Basics/data/wiki_number_of_billionaires.csv')
...
df_parquet = spark.read.schema(people_schema).parquet('/home/jovyan/work/3_Spark_Basics/data_parquet')
df_parquet_filtered = df_parquet.filter(col('nationality') == 'Russia')

df_last = df_last.union(df_parquet_filtered)
df_last.write.option("header", 'true').mode('overwrite').csv("/home/jovyan/work/3_Spark_Basics/1.csv")
...
with open('/home/jovyan/work/3_Spark_Basics/plans.txt', 'w') as file:
    file.write(plans)

with open('/home/jovyan/work/3_Spark_Basics/plans1.txt', 'w') as file:
    file.write(plans_1)
